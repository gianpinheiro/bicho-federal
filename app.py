...
if st.button(f"Consultar {premio_sel}", type="primary", use_container_width=True):
    with st.spinner(f"Analisando {premio_sel}..."):
        ...
        ultima_info = {} # bicho_id -> {data, posicao}
        ...
        for bicho_id in range(1, 26):
            ...
            if info:
                ...
                dias = (hoje - datetime.strptime(linhas[-1][0]['value'], "%Y-%m-%d")).days if linhas else 999
                concursos = len(linhas)
                ultima = "Nunca"

            lista.append({
                "Bicho": f"{bicho_id:02d} - {nome}",
                "Dias Atrasado": dias,
                "Concursos Atrasados": concursos,
                "Última vez": ultima
            })
